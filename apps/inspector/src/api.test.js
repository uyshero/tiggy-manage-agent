...
test("Inspector Session, Artifact, Intervention, and Observability reads use v2 SDK services", async () => {
  ...
  globalThis.fetch = async (path, options) => {
    ...
    if (url.endsWith("/task-plans")) return { ok: true, json: async () => ({ plans: [{ id: "plan/1", status: "completed", items: [] }] }) };
  ...
  try {
    ...
    const planHistory = await taskPlans("session/1", { signal: controller.signal });
    ...
    assert.equal(planHistory.plans[0].id, "plan/1");
  ...
  assert.deepEqual(calls.map((call) => call.url), [
    ...
    "http://localhost/v2/sessions/session%2F1/task-plans",
