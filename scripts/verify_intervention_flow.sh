...
types = [event.get("type") for event in events]
sys.exit(0 if "agent.message" in types and "session.status_idle" in types else 1)
'; then
      printf '%s\n' "$events_json"
      return 0
    fi
...
echo "Upserting provider/model from $DOTENV_PATH"
...
system_prompt='You are testing TMA tool approval. When the user says exactly APPROVAL_TEST_RUN_COMMAND, you must call the tool tma.local_system.run_command with command "sh" and args ["-c","pwd"]. Do not answer in text before calling the tool.'
...
case "$DECISION" in
  ...
  approve)
    ...
    echo "Waiting for agent.message + idle"
    events_json="$(wait_for_idle_agent "$session_id")"
