...
class AgentsService extends ServiceBase {
  ...
  listByApplication(query = {}, signal) {
    ...
    return this.transport.requestJSON("GET", path, void 0, signal ? { signal } : {}).then((value) => value.agents);
  }
  get(agentId, signal) {
...
class EnvironmentsService extends ServiceBase {
  ...
  listByApplication(query = {}, signal) {
    ...
    return this.transport.requestJSON("GET", path, void 0, signal ? { signal } : {}).then((value) => value.environments);
  }
...
class EvaluationsService extends ServiceBase {
  ...
  listDatasets(workspaceId, signal) {
    ...
    return this.transport.requestJSON("GET", path, void 0, signal ? { signal } : {}).then((value) => value.datasets);
  }
  ...
  listExperiments(workspaceId, limit, signal) {
    ...
    return this.transport.requestJSON("GET", path, void 0, signal ? { signal } : {}).then((value) => value.experiments);
  }
...
class EventSubscriptionsService extends ServiceBase {
  ...
  list(appId, signal) {
    ...
    return this.transport.requestJSON("GET", path, void 0, signal ? { signal } : {}).then((value) => value.items);
  }
  create(request, signal) {
...
class SkillsService extends ServiceBase {
  ...
  list(query = {}, signal) {
    const path = withQuery("/v2/skills", { workspace_id: query.workspaceId, app_id: query.appId, external_ref: query.externalRef, include_archived: query.includeArchived || void 0 });
...
class MarketplaceService extends ServiceBase {
  ...
  getPolicyVersion(policyId, version, signal) {
    return this.transport.requestJSON("GET", resourcePath(`${policyPath$1(policyId)}/versions`, version), void 0, signal ? { signal } : {});
  }
...
class RunsService extends ServiceBase {
  constructor(transport, interventions) {
    super(transport);
...
class RunHandle {
  constructor(service, interventions, run, initialEvents = [], created = true) {
...
class TracesService extends ServiceBase {
  ...
  get(traceId, signal) {
    return this.transport.requestJSON("GET", tracePath(traceId), void 0, signal ? { signal } : {});
  }
  ...
  getSpan(traceId, spanId, signal) {
    return this.transport.requestJSON("GET", resourcePath(`${tracePath(traceId)}/spans`, spanId), void 0, signal ? { signal } : {});
  }
...
async function catalogs(signal) {
...
function runs(sessionID, signal) {
  if (!sessionID) return Promise.resolve([]);
  return coreSDK.runs.list(sessionID, signal);
}
async function analyze(leftSessionID, leftTurnID, rightSessionID, rightTurnID, signal) {
...
function listRubrics(workspaceID, signal) {
  return coreSDK.evaluations.listRubrics(workspaceID, signal);
}
function createRubric(workspaceID, criteria, signal) {
...
function listEvaluations(leftSessionID, leftTurnID, rightSessionID, rightTurnID, signal) {
...
function saveEvaluation({ leftSessionID, leftTurnID, rightSessionID, rightTurnID, rubricID, evaluation }, signal) {
...
function autoEvaluate({ leftSessionID, leftTurnID, rightSessionID, rightTurnID, rubricID }, signal) {
...
function listDatasets(workspaceID, signal) {
  return coreSDK.evaluations.listDatasets(workspaceID, signal);
}
function createDataset(workspaceID, { name, description, items }, signal) {
...
function listExperiments(workspaceID, signal) {
  return coreSDK.evaluations.listExperiments(workspaceID, 30, signal);
}
function createExperiment({ name, datasetID, rubricID, leftTemplateSessionID, rightTemplateSessionID }, signal) {
...
function reconcileExperiment(experimentID, signal) {
  return coreSDK.evaluations.reconcileExperiment(experimentID, signal);
}
const spaceAPI = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  analyze,
  autoEvaluate,
  catalogs,
...
function field(record, keys) {
  for (const key of keys) {
    if ((record == null ? void 0 : record[key]) !== void 0 && record[key] !== null) return record[key];
  }
  return "";
}
function baseName(filename) {
  return String(filename || "导入数据集").replace(/^.*[\\/]/, "").replace(/\.[^.]+$/, "").trim() || "导入数据集";
}
...
function normalizeItems(records) {
  ...
  const seenPrompts = /* @__PURE__ */ new Map();
  return records.map((record, index) => {
    ...
    if ([...prompt].length > 2e4) throw new Error(`第 ${rowNumber} 行的提示词不能超过 20000 个字符。`);
    if ([...expectedOutput].length > 2e4) throw new Error(`第 ${rowNumber} 行的期望结果不能超过 20000 个字符。`);
...
function datasetItems(dataset) {
  return ((dataset == null ? void 0 : dataset.items) || []).map((item) => ({
...
function serializeDataset(dataset, format = "json") {
  ...
  if (format === "csv") {
    return {
      ...
      filename: `${safeFilename((dataset == null ? void 0 : dataset.name) || "dataset")}.csv`,
      mimeType: "text/csv;charset=utf-8"
    };
  }
  return {
    content: JSON.stringify({ name: (dataset == null ? void 0 : dataset.name) || "", description: (dataset == null ? void 0 : dataset.description) || "", items }, null, 2),
    filename: `${safeFilename((dataset == null ? void 0 : dataset.name) || "dataset")}.json`,
    mimeType: "application/json;charset=utf-8"
  };
}
function serializeDatasetTemplate() {
  return {
...
function serializeExperiment(experiment, format = "json") {
  const items = ((experiment == null ? void 0 : experiment.items) || []).map((item) => ({
    ...
    comparison_url: item.left_session_id && item.right_session_id ? `/space#left=${encodeURIComponent(item.left_session_id)}&left_turn=${encodeURIComponent(item.left_turn_id || "")}&right=${encodeURIComponent(item.right_session_id)}&right_turn=${encodeURIComponent(item.right_turn_id || "")}` : ""
  }));
  if (format === "csv") {
    return {
      content: Papa.unparse(items.map((item) => ({ ...item, tags: item.tags.join("|") }))),
      filename: `${safeFilename((experiment == null ? void 0 : experiment.name) || "experiment")}-results.csv`,
      mimeType: "text/csv;charset=utf-8"
    };
  }
  return {
    content: JSON.stringify({
      id: (experiment == null ? void 0 : experiment.id) || "",
      name: (experiment == null ? void 0 : experiment.name) || "",
      status: (experiment == null ? void 0 : experiment.status) || "",
      dataset_id: (experiment == null ? void 0 : experiment.dataset_id) || "",
      rubric_id: (experiment == null ? void 0 : experiment.rubric_id) || "",
      summary: (experiment == null ? void 0 : experiment.summary) || {},
      items
    }, null, 2),
    filename: `${safeFilename((experiment == null ? void 0 : experiment.name) || "experiment")}-results.json`,
    mimeType: "application/json;charset=utf-8"
  };
}
...
function localizedCriterion(criterion) {
  return criterionTranslations.get(criterion == null ? void 0 : criterion.id) || criterion;
}
...
function evaluationFromRubric(rubric, persisted) {
  const scores = new Map(((persisted == null ? void 0 : persisted.scores) || []).map((score) => [score.criterion_id, score]));
  const criteria = Array.isArray(rubric == null ? void 0 : rubric.criteria) && rubric.criteria.length ? rubric.criteria : defaultCriteria;
  return normalizeEvaluation({
    conclusion: (persisted == null ? void 0 : persisted.conclusion) || "inconclusive",
    notes: (persisted == null ? void 0 : persisted.notes) || "",
    savedAt: (persisted == null ? void 0 : persisted.created_at) || "",
    criteria: criteria.map((criterion) => {
      var _a2, _b;
      return {
        ...criterion,
        leftScore: ((_a2 = scores.get(criterion.id)) == null ? void 0 : _a2.left_score) ?? 3,
        rightScore: ((_b = scores.get(criterion.id)) == null ? void 0 : _b.right_score) ?? 3
      };
    })
  });
}
...
function averageScore(criteria, side) {
...
function summarizeEvidence(evidence = {}) {
  ...
  const usage = ((_a2 = comparison.usage) == null ? void 0 : _a2.summary) || {};
  ...
  return {
    ...
    summary: ((_b = evidence.summary) == null ? void 0 : _b.summary_text) || trace.summary || "",
    ...
    spanCount: numberOr(stats.span_count, (_c = trace.spans) == null ? void 0 : _c.length),
    artifactCount: Array.isArray(comparison.artifacts) ? comparison.artifacts.length : numberOr(stats.artifact_count),
    criticalPathMS: numberOr((_d = trace.graph) == null ? void 0 : _d.critical_path_duration_ms),
    traceID: trace.trace_id || "",
    turnID: trace.turn_id || "",
    status: trace.status || ((_e = comparison.session) == null ? void 0 : _e.status) || "unknown",
    steps,
    events,
    toolSteps,
    errorSteps,
...
function formatDuration(value) {
  ...
  if (ms < 1e3) return `${ms.toLocaleString()} ms`;
...
function Pill({ value }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `pill ${pillTone(value)}`, children: statusLabel(value) });
}
function IconButton({ label, children, ...props }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "icon-button", type: "button", "aria-label": label, title: label, ...props, children });
}
...
function Metric({ label, left, right, format = (value) => Number(value || 0).toLocaleString(), unit = "", lowerIsBetter = false }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "metric-cell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "metric-label", children: label }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "metric-values", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: format(left) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: format(right) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("small", { className: `delta ${direction}`, children: [
      formatDelta(delta, unit),
      "，B - A"
    ] })
  ] });
}
function RunPicker({ label, sessionID, turnID, sessions, runs: runs2, onSessionChange, onTurnChange, loading }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "session-picker", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: label }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { "aria-label": `${label} 会话`, value: sessionID, onChange: (event) => onSessionChange(event.target.value), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "选择会话" }),
      sessions.map((session) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: session.id, children: sessionLabel(session) }, session.id))
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { "aria-label": `${label} 轮次`, value: turnID, disabled: !sessionID || loading, onChange: (event) => onTurnChange(event.target.value), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: loading ? "正在加载轮次..." : "选择轮次" }),
      runs2.map((run) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: run.id, children: runLabel(run) }, run.id))
    ] })
  ] });
}
function EvidenceHeader({ label, summary }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "evidence-header", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "side-label", children: label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: summary.session.title || summary.session.id || "会话" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "meta-line", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Pill, { value: summary.status }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          summary.provider,
          " / ",
          summary.model
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: summary.turnID || "最新轮次" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("a", { className: "inspector-link", href: inspectorURL, target: "_blank", rel: "noreferrer", children: [
      "检查器 ",
...
function TextEvidence({ title, left, right, empty }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-heading", children: /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: title }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "evidence-columns", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-evidence", children: /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { children: left || empty }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-evidence", children: /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { children: right || empty }) })
    ] })
  ] });
}
function StepList({ summary }) {
  const steps = summary.steps.slice(-18);
  if (!steps.length) return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-row", children: "暂无追踪证据。" });
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "step-list", children: steps.map((step, index) => {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `step-row ${error ? "error" : tool ? "tool" : ""}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "step-index", children: step.seq || index + 1 }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: step.type || "运行事件" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: statusLabel(step.message || step.summary || modelToolName(step.identifier, step.api_name) || "无详细信息") })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "step-meta", children: [
        step.outcome ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pill, { value: step.outcome }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDuration(step.duration_ms) })
      ] })
    ] }, `${step.seq || index}-${step.type}`);
  }) });
}
function TraceEvidence({ left, right }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-heading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "执行证据" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "每侧最近 18 个步骤" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "evidence-columns trace-columns", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(StepList, { summary: left }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(StepList, { summary: right })
    ] })
  ] });
}
function Diagnostics({ left, right }) {
  const render = (summary) => {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "diagnostic-side", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "diagnostic-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "工具调用" }),
        tools.length ? tools.map((step, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "diagnostic-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: modelToolName(step.identifier, step.api_name) || step.message || "工具调用" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: step.call_id || `seq ${step.seq || "-"}` })
        ] }, `${step.call_id || step.seq || index}-tool`)) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-row compact", children: "暂无工具调用。" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "diagnostic-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "失败记录" }),
        summary.errorSteps.length ? summary.errorSteps.map((step, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "diagnostic-row failure", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: step.message || step.summary || step.type }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: step.outcome || step.span_status || `seq ${step.seq || "-"}` })
        ] }, `${step.seq || index}-error`)) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-row compact", children: "暂无失败记录。" })
      ] })
    ] });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-heading", children: /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "诊断信息" }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "evidence-columns", children: [
      render(left),
      render(right)
    ] })
...
function ScoreControl({ value, onChange, label }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "score-control", role: "radiogroup", "aria-label": label, children: [1, 2, 3, 4, 5].map((score) => /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
...
function Rubric({ evaluation, setEvaluation, onSave, onAuto, onReset, onExport, saved, saving, judging }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section rubric-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-heading rubric-heading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "评分评估" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: saved ? `已保存于 ${formatTime(saved)}` : "尚未保存" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: "重置评分", onClick: onReset, children: /* @__PURE__ */ jsxRuntimeExports.jsx(RotateCcw, { size: 16 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: "导出评估", onClick: onExport, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Download, { size: 16 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "judge-action", type: "button", disabled: saving || judging, onClick: onAuto, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { size: 16 }),
          " ",
          judging ? "评测中" : "自动评测"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "primary-action", type: "button", disabled: saving || judging, onClick: onSave, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rubric-summary", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "A 平均分" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: leftAverage.toFixed(2) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "B 平均分" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: rightAverage.toFixed(2) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "B - A" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: rightAverage > leftAverage ? "positive" : rightAverage < leftAverage ? "negative" : "", children: formatDelta(Number((rightAverage - leftAverage).toFixed(2))) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rubric-table", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rubric-table-head", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "评分项" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "运行 A" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "运行 B" })
      ] }),
      evaluation.criteria.map((criterion, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rubric-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: localizedCriterion(criterion).name }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: localizedCriterion(criterion).description })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(ScoreControl, { value: criterion.leftScore, label: `${localizedCriterion(criterion).name}，运行 A 评分`, onChange: (leftScore) => updateCriterion(index, { leftScore }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(ScoreControl, { value: criterion.rightScore, label: `${localizedCriterion(criterion).name}，运行 B 评分`, onChange: (rightScore) => updateCriterion(index, { rightScore }) })
      ] }, criterion.id))
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "evaluation-footer", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: "评估结论" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "conclusion-control", children: [{ value: "left", label: "A 更优" }, { value: "right", label: "B 更优" }, { value: "tie", label: "持平" }, { value: "inconclusive", label: "无法判断" }].map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: evaluation.conclusion === option.value ? "active" : "", type: "button", onClick: () => setEvaluation((current) => ({ ...current, conclusion: option.value })), children: option.label }, option.value)) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "notes-field", children: [
        "评估备注",
        /* @__PURE__ */ jsxRuntimeExports.jsx("textarea", { value: evaluation.notes, onChange: (event) => setEvaluation((current) => ({ ...current, notes: event.target.value })), placeholder: "填写判断依据或补充说明" })
      ] })
    ] })
  ] });
}
function EvaluationHistory({ evaluations }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section history-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-heading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "评估历史" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        "已保存 ",
        evaluations.length,
        " 条"
    ...
    evaluations.length ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "history-list", children: evaluations.map((item) => {
      ...
      const source = item.evaluation_type === "auto" ? `自动 · ${item.judge_provider || "未知 Provider"} / ${item.judge_model || "未知模型"}` : "人工";
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "history-row", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Clock3, { size: 15, "aria-hidden": "true" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: item.notes || "无备注" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "history-scores", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
            "A ",
            left.toFixed(2)
          ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("time", { children: formatTime(item.created_at) })
      ] }, item.id);
    }) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-row", children: "这组运行尚无已保存的评估。" })
  ] });
}
...
function BatchWorkspace({ catalog, initialLeftID, initialRightID }) {
  var _a2, _b;
  const workspaceID = ((_a2 = catalog.sessions.find((session) => session.id === initialLeftID)) == null ? void 0 : _a2.workspace_id) || ((_b = catalog.sessions[0]) == null ? void 0 : _b.workspace_id) || "";
  const templateSessions = reactExports.useMemo(
  ...
  const [rubrics, setRubrics] = reactExports.useState([]);
  const [experiments, setExperiments] = reactExports.useState([]);
  const [datasetID, setDatasetID] = reactExports.useState("");
  const [rubricID, setRubricID] = reactExports.useState("");
  const [experimentID, setExperimentID] = reactExports.useState("");
  const [leftTemplateID, setLeftTemplateID] = reactExports.useState(initialLeftID || "");
  const [rightTemplateID, setRightTemplateID] = reactExports.useState(initialRightID || "");
  const [datasetName, setDatasetName] = reactExports.useState("");
  const [datasetDescription, setDatasetDescription] = reactExports.useState("");
  const [datasetRows, setDatasetRows] = reactExports.useState([{ prompt: "", expectedOutput: "", tags: "" }]);
  const [experimentName, setExperimentName] = reactExports.useState("");
  const [loading, setLoading] = reactExports.useState(true);
  const [creatingDataset, setCreatingDataset] = reactExports.useState(false);
  ...
  const [datasetExportFormat, setDatasetExportFormat] = reactExports.useState("json");
  const [experimentExportFormat, setExperimentExportFormat] = reactExports.useState("json");
  const [error, setError] = reactExports.useState("");
  const refreshRef = reactExports.useRef(false);
  const datasetFileRef = reactExports.useRef(null);
  ...
  const mergeExperiment = reactExports.useCallback((updated) => {
  ...
  reactExports.useEffect(() => {
    ...
    setError("");
    Promise.all([
      listDatasets(workspaceID, controller.signal),
      listRubrics(workspaceID, controller.signal),
      listExperiments(workspaceID, controller.signal)
      ...
      setDatasetID((current) => {
        var _a3;
        return current || ((_a3 = nextDatasets[0]) == null ? void 0 : _a3.id) || "";
      });
      setRubricID((current) => {
        var _a3;
        return current || ((_a3 = nextRubrics[0]) == null ? void 0 : _a3.id) || "";
      });
      setExperimentID((current) => {
        var _a3;
        return current || ((_a3 = nextExperiments[0]) == null ? void 0 : _a3.id) || "";
      });
    }).catch((loadError) => {
      if ((loadError == null ? void 0 : loadError.name) !== "AbortError") setError(String((loadError == null ? void 0 : loadError.message) || loadError));
  ...
  reactExports.useEffect(() => {
    ...
    if (!templateSessions.some((session) => session.id === leftTemplateID)) setLeftTemplateID(((_a3 = templateSessions[0]) == null ? void 0 : _a3.id) || "");
    if (!templateSessions.some((session) => session.id === rightTemplateID) || rightTemplateID === leftTemplateID) {
      setRightTemplateID(((_b2 = templateSessions.find((session) => session.id !== leftTemplateID)) == null ? void 0 : _b2.id) || "");
    }
  }, [templateSessions, leftTemplateID, rightTemplateID]);
  const refreshExperiment = reactExports.useCallback(async (targetID = experimentID) => {
    if (!targetID || refreshRef.current) return;
    refreshRef.current = true;
    setRefreshing(true);
    try {
      const updated = await reconcileExperiment(targetID);
      mergeExperiment(updated);
    } catch (refreshError) {
      setError(String((refreshError == null ? void 0 : refreshError.message) || refreshError));
    } finally {
  ...
  reactExports.useEffect(() => {
    if (!selectedExperiment || selectedExperiment.status !== "running") return void 0;
    const timer = window.setInterval(() => refreshExperiment(selectedExperiment.id), 1500);
    return () => window.clearInterval(timer);
  }, [selectedExperiment == null ? void 0 : selectedExperiment.id, selectedExperiment == null ? void 0 : selectedExperiment.status, refreshExperiment]);
  ...
  async function previewDatasetFile(event) {
    ...
    try {
      ...
      setError(String((importError == null ? void 0 : importError.message) || importError));
    }
  }
  ...
  async function createDataset$1() {
    ...
    setError("");
    try {
      const created = await createDataset(workspaceID, {
      ...
      setError(String((createError == null ? void 0 : createError.message) || createError));
    } finally {
  ...
  async function startExperiment() {
    ...
    try {
      ...
      if (!activeRubricID) {
        const createdRubric = await createRubric(workspaceID, createEvaluation().criteria);
      ...
      const created = await createExperiment({
        name: experimentName.trim() || `${(activeDataset == null ? void 0 : activeDataset.name) || "批量"} 对比`,
        datasetID,
        rubricID: activeRubricID,
        leftTemplateSessionID: leftTemplateID,
        rightTemplateSessionID: rightTemplateID
      });
      mergeExperiment(created);
      setExperimentName("");
    } catch (startError) {
      setError(String((startError == null ? void 0 : startError.message) || startError));
    } finally {
      setStarting(false);
    }
  }
  if (loading) return /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "batch-main", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-state", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(FlaskConical, { size: 28 }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "正在加载批量实验" })
  ] }) });
  const summary = (selectedExperiment == null ? void 0 : selectedExperiment.summary) || {};
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "batch-main", children: [
    error ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "error-banner", children: error }) : null,
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "batch-setup-grid", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section dataset-builder", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-heading", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-title", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Database, { size: 16 }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "评测数据集" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
            datasets.length,
            " 个"
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { className: "visually-hidden", ref: datasetFileRef, type: "file", "aria-label": "选择数据集文件", accept: ".csv,.json,text/csv,application/json", onChange: previewDatasetFile }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dataset-file-toolbar", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "file-toolbar-group", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "secondary-action", type: "button", onClick: () => {
              var _a3;
              return (_a3 = datasetFileRef.current) == null ? void 0 : _a3.click();
            }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(Upload, { size: 15 }),
              " 导入文件"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "secondary-action", type: "button", onClick: () => downloadTextFile(serializeDatasetTemplate()), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(FileDown, { size: 15 }),
              " 下载模板"
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "file-toolbar-group export-toolbar", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { className: "export-format-select", "aria-label": "数据集导出格式", value: datasetExportFormat, onChange: (event) => setDatasetExportFormat(event.target.value), children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: selectedDataset ? `导出数据集 ${selectedDataset.name}` : "导出选中数据集", disabled: !selectedDataset, onClick: exportSelectedDataset, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Download, { size: 15 }) })
          ] })
        ] }),
        importPreview ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "import-preview", "aria-label": "导入预览", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "import-preview-heading", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: importPreview.name }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-actions", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "secondary-action", type: "button", onClick: () => setImportPreview(null), children: "取消" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-action", type: "button", onClick: applyImportPreview, children: "载入编辑" })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "import-preview-list", children: importPreview.items.map((item, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "import-preview-row", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: index + 1 }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: item.prompt }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: item.expectedOutput || "无期望结果" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: item.tags.join(" · ") || "无标签" })
          ] }, `${index}-${item.prompt}`)) })
        ] }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "batch-form-grid", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "数据集名称",
            /* @__PURE__ */ jsxRuntimeExports.jsx("input", { value: datasetName, onChange: (event) => setDatasetName(event.target.value) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "描述",
            /* @__PURE__ */ jsxRuntimeExports.jsx("input", { value: datasetDescription, onChange: (event) => setDatasetDescription(event.target.value) })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dataset-row-head", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "提示词" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "期望结果" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "标签" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", {})
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dataset-rows", children: datasetRows.map((row, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dataset-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("textarea", { "aria-label": `样本 ${index + 1} 提示词`, value: row.prompt, onChange: (event) => updateDatasetRow(index, { prompt: event.target.value }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("textarea", { "aria-label": `样本 ${index + 1} 期望结果`, value: row.expectedOutput, onChange: (event) => updateDatasetRow(index, { expectedOutput: event.target.value }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { "aria-label": `样本 ${index + 1} 标签`, value: row.tags, onChange: (event) => updateDatasetRow(index, { tags: event.target.value }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: `删除样本 ${index + 1}`, disabled: datasetRows.length === 1, onClick: () => setDatasetRows((current) => current.filter((_, rowIndex) => rowIndex !== index)), children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { size: 15 }) })
        ] }, index)) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "builder-actions", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "secondary-action", type: "button", disabled: datasetRows.length >= 20, onClick: () => setDatasetRows((current) => [...current, { prompt: "", expectedOutput: "", tags: "" }]), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { size: 15 }),
            " 添加样本"
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "primary-action", type: "button", disabled: !datasetName.trim() || !datasetRows.some((row) => row.prompt.trim()) || creatingDataset, onClick: createDataset$1, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section experiment-launcher", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-heading", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-title", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(FlaskConical, { size: 16 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "实验配置" })
        ] }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "experiment-fields", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "实验名称",
            /* @__PURE__ */ jsxRuntimeExports.jsx("input", { value: experimentName, onChange: (event) => setExperimentName(event.target.value) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "数据集",
            /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: datasetID, onChange: (event) => setDatasetID(event.target.value), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "选择数据集" }),
              datasets.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsxs("option", { value: item.id, children: [
                item.name,
                " · ",
                item.items.length,
                " 条"
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "评分标准",
            /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: rubricID, onChange: (event) => setRubricID(event.target.value), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "默认评分标准" }),
              rubrics.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsxs("option", { value: item.id, children: [
                item.name,
                " · v",
                item.revision
              ] }, item.id))
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "模板会话 A",
            /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: leftTemplateID, onChange: (event) => setLeftTemplateID(event.target.value), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "选择会话" }),
              templateSessions.map((session) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: session.id, children: sessionLabel(session) }, session.id))
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            "模板会话 B",
            /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: rightTemplateID, onChange: (event) => setRightTemplateID(event.target.value), children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "选择会话" }),
              templateSessions.map((session) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: session.id, children: sessionLabel(session) }, session.id))
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "primary-action launch-action", type: "button", disabled: !datasetID || !leftTemplateID || !rightTemplateID || leftTemplateID === rightTemplateID || starting, onClick: startExperiment, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "analysis-section experiment-results", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-heading experiment-heading", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-title", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(FlaskConical, { size: 16 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "实验结果" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "experiment-select-actions", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { className: "experiment-picker", "aria-label": "实验", value: (selectedExperiment == null ? void 0 : selectedExperiment.id) || "", onChange: (event) => setExperimentID(event.target.value), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "选择实验" }),
            experiments.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsxs("option", { value: item.id, children: [
              item.name,
              " · ",
              experimentStatusLabel(item.status)
            ] }, item.id))
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { className: "export-format-select", "aria-label": "实验结果导出格式", value: experimentExportFormat, onChange: (event) => setExperimentExportFormat(event.target.value), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "json", children: "JSON" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "csv", children: "CSV" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: "导出实验结果", disabled: !selectedExperiment, onClick: exportSelectedExperiment, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Download, { size: 15 }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: "刷新实验结果", disabled: !selectedExperiment || refreshing, onClick: () => refreshExperiment(selectedExperiment == null ? void 0 : selectedExperiment.id), children: /* @__PURE__ */ jsxRuntimeExports.jsx(RefreshCw, { size: 15, className: refreshing ? "spinning" : "" }) })
        ] })
      ] }),
      selectedExperiment ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "experiment-summary", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "进度" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
              Number(summary.completed || 0) + Number(summary.failed || 0),
              " / ",
              summary.total || 0
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "A 胜" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: summary.left_wins || 0 })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "B 胜" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: summary.right_wins || 0 })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "持平" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: summary.ties || 0 })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "A 平均分" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: Number(summary.left_average || 0).toFixed(2) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "B 平均分" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: Number(summary.right_average || 0).toFixed(2) })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "experiment-table", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "experiment-table-head", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "样本" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "状态" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "A / B" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "结论" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "证据" })
          ] }),
          selectedExperiment.items.map((item) => {
            var _a3;
            return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "experiment-row", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
                  "#",
                  item.item_index + 1,
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: ((_a3 = item.tags) == null ? void 0 : _a3.join(" · ")) || item.expected_output || "" }),
                item.error_message ? /* @__PURE__ */ jsxRuntimeExports.jsx("small", { className: "row-error", children: item.error_message }) : null
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Pill, { value: item.status }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
                Number(item.left_average || 0).toFixed(2),
                " / ",
                Number(item.right_average || 0).toFixed(2)
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: item.conclusion ? conclusionLabel(item.conclusion) : "-" }),
              item.left_session_id && item.right_session_id ? /* @__PURE__ */ jsxRuntimeExports.jsxs("a", { className: "inspector-link", target: "_blank", rel: "noreferrer", href: `/space#left=${encodeURIComponent(item.left_session_id)}&left_turn=${encodeURIComponent(item.left_turn_id)}&right=${encodeURIComponent(item.right_session_id)}&right_turn=${encodeURIComponent(item.right_turn_id)}`, children: [
                "查看对比 ",
      ...
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-row", children: "暂无实验。" })
    ] })
  ] });
}
function EmptyState({ loading, error }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "empty-state", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { size: 28, "aria-hidden": "true" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: loading ? "正在加载对比" : error ? "分析失败" : "请选择两组运行" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: error || (loading ? "正在读取运行证据..." : "") })
  ] });
}
function App() {
  const [view, setView] = reactExports.useState("comparison");
  const [catalog, setCatalog] = reactExports.useState({ agents: [], sessions: [] });
  ...
  const [leftTurnID, setLeftTurnID] = reactExports.useState("");
  const [rightTurnID, setRightTurnID] = reactExports.useState("");
  const [leftRuns, setLeftRuns] = reactExports.useState([]);
  const [rightRuns, setRightRuns] = reactExports.useState([]);
  const [runLoading, setRunLoading] = reactExports.useState({ left: false, right: false });
  const [analysis, setAnalysis] = reactExports.useState(null);
  const [loading, setLoading] = reactExports.useState(false);
  const [catalogLoading, setCatalogLoading] = reactExports.useState(true);
  const [error, setError] = reactExports.useState("");
  const [evaluation, setEvaluation] = reactExports.useState(createEvaluation);
  const [rubric, setRubric] = reactExports.useState(null);
  const [evaluations, setEvaluations] = reactExports.useState([]);
  const [saving, setSaving] = reactExports.useState(false);
  const [judging, setJudging] = reactExports.useState(false);
  const [savedAt, setSavedAt] = reactExports.useState("");
  const requestRef = reactExports.useRef(null);
  const runRequestRef = reactExports.useRef({ left: null, right: null });
  const sessions = reactExports.useMemo(() => {
    ...
    return [...items].sort((left2, right2) => String(right2.updated_at || right2.created_at || "").localeCompare(String(left2.updated_at || left2.created_at || "")));
  }, [agentID, catalog.sessions]);
  const load = reactExports.useCallback(async (nextLeft = leftID, nextLeftTurn = leftTurnID, nextRight = rightID, nextRightTurn = rightTurnID) => {
    var _a2;
    ...
    (_a2 = requestRef.current) == null ? void 0 : _a2.abort();
    const controller = new AbortController();
    ...
    try {
      const result = await analyze(nextLeft, nextLeftTurn, nextRight, nextRightTurn, controller.signal);
      if (requestRef.current !== controller) return;
      const workspaceID = result.comparison.left.session.workspace_id;
      const [availableRubrics, history] = await Promise.all([
        listRubrics(workspaceID, controller.signal),
        listEvaluations(nextLeft, nextLeftTurn, nextRight, nextRightTurn, controller.signal)
      ]);
      if (requestRef.current !== controller) return;
      const latest = history[0];
      const activeRubric = availableRubrics.find((item) => item.id === (latest == null ? void 0 : latest.rubric_id)) || availableRubrics.find((item) => item.name === "TMA Space default") || availableRubrics[0] || null;
      setAnalysis(result);
      setRubric(activeRubric);
      setEvaluations(history);
      const persisted = (latest == null ? void 0 : latest.rubric_id) === (activeRubric == null ? void 0 : activeRubric.id) ? latest : null;
      const nextEvaluation = evaluationFromRubric(activeRubric || (latest == null ? void 0 : latest.rubric_snapshot), persisted);
  ...
  async function loadRuns(side, sessionID, preferredTurnID = "") {
    var _a2, _b;
    (_a2 = runRequestRef.current[side]) == null ? void 0 : _a2.abort();
    if (!sessionID) {
      if (side === "left") {
        setLeftRuns([]);
        setLeftTurnID("");
    ...
    try {
      const items = [...await runs(sessionID, controller.signal)].sort((left2, right2) => String(right2.started_at || "").localeCompare(String(left2.started_at || "")));
      const turnID = items.some((run) => run.id === preferredTurnID) ? preferredTurnID : ((_b = items[0]) == null ? void 0 : _b.id) || "";
      if (side === "left") {
        setLeftRuns(items);
        setLeftTurnID(turnID);
      } else {
        setRightRuns(items);
        setRightTurnID(turnID);
      }
      return { runs: items, turnID };
    } catch (runError) {
      if ((runError == null ? void 0 : runError.name) !== "AbortError") setError(String((runError == null ? void 0 : runError.message) || runError));
      return { runs: [], turnID: "" };
    } finally {
  ...
  reactExports.useEffect(() => {
    ...
    catalogs(controller.signal).then(async (result) => {
      var _a2, _b;
      const sorted = [...result.sessions].sort((left2, right2) => String(right2.updated_at || right2.created_at || "").localeCompare(String(left2.updated_at || left2.created_at || "")));
      setCatalog({ ...result, sessions: sorted });
      const hashLeft = params.get("left") || "";
      const hashRight = params.get("right") || "";
      const nextLeft = sorted.some((session) => session.id === hashLeft) ? hashLeft : ((_a2 = sorted[0]) == null ? void 0 : _a2.id) || "";
      const nextRight = sorted.some((session) => session.id === hashRight) ? hashRight : ((_b = sorted.find((session) => {
        var _a3;
        return session.id !== nextLeft && session.workspace_id === ((_a3 = sorted.find((item) => item.id === nextLeft)) == null ? void 0 : _a3.workspace_id);
      })) == null ? void 0 : _b.id) || "";
      setLeftID(nextLeft);
      ...
      if ((catalogError == null ? void 0 : catalogError.name) !== "AbortError") setError(String((catalogError == null ? void 0 : catalogError.message) || catalogError));
      setCatalogLoading(false);
    });
    return () => controller.abort();
  }, []);
  ...
  async function changeAgent(nextAgentID) {
    ...
    const nextLeft = ((_a2 = filtered[0]) == null ? void 0 : _a2.id) || "";
    const nextRight = ((_b = filtered.find((session) => {
      var _a3;
      return session.id !== nextLeft && session.workspace_id === ((_a3 = filtered[0]) == null ? void 0 : _a3.workspace_id);
    })) == null ? void 0 : _b.id) || "";
    setLeftID(nextLeft);
    setRightID(nextRight);
    clearComparison();
  ...
  async function saveEvaluation$1() {
    if (!analysis || saving) return;
    setSaving(true);
    setError("");
    try {
      const workspaceID = analysis.comparison.left.session.workspace_id;
      const activeRubric = rubric || await createRubric(workspaceID, evaluation.criteria);
      ...
      setError(String((saveError == null ? void 0 : saveError.message) || saveError));
    } finally {
      setSaving(false);
    }
  }
  async function autoEvaluate$1() {
    ...
    try {
      ...
      const activeRubric = rubric || await createRubric(workspaceID, evaluation.criteria);
      if (!rubric) setRubric(activeRubric);
      const saved = await autoEvaluate({
        leftSessionID: leftID,
        leftTurnID,
        rightSessionID: rightID,
        rightTurnID,
      ...
      setError(String((judgeError == null ? void 0 : judgeError.message) || judgeError));
    } finally {
      setJudging(false);
    }
  }
  ...
  function exportEvaluation() {
    const payload = JSON.stringify({
      ...
      rightSessionID: rightID,
      rightTurnID,
      rubricID: (rubric == null ? void 0 : rubric.id) || "",
      ...evaluation
    }, null, 2);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-shell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "header-start", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "brand", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Scale, { size: 20, "aria-hidden": "true" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "TMA Space" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "运行分析" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "view-tabs", role: "tablist", "aria-label": "Space 视图", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", role: "tab", "aria-selected": view === "comparison", className: view === "comparison" ? "active" : "", onClick: () => setView("comparison"), children: "单次分析" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", role: "tab", "aria-selected": view === "batch", className: view === "batch" ? "active" : "", onClick: () => setView("batch"), children: "批量实验" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "header-links", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: "/app", children: "工作台" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: "/inspector", children: "检查器" })
      ] })
    ] }),
    view === "comparison" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "comparison-toolbar", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "agent-filter", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "智能体" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: agentID, onChange: (event) => changeAgent(event.target.value), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "全部智能体" }),
            catalog.agents.map((agent) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: agent.id, children: agent.name || agent.id }, agent.id))
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(RunPicker, { label: "运行 A", sessionID: leftID, turnID: leftTurnID, sessions, runs: leftRuns, loading: runLoading.left, onSessionChange: (value) => changeSession("left", value), onTurnChange: (value) => {
          setLeftTurnID(value);
          clearComparison();
        } }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(IconButton, { label: "交换两侧运行", disabled: !leftTurnID || !rightTurnID, onClick: swap, children: /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowLeftRight, { size: 17 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(RunPicker, { label: "运行 B", sessionID: rightID, turnID: rightTurnID, sessions, runs: rightRuns, loading: runLoading.right, onSessionChange: (value) => changeSession("right", value), onTurnChange: (value) => {
          setRightTurnID(value);
          clearComparison();
        } }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "analyze-button", type: "button", disabled: loading || !leftTurnID || !rightTurnID, onClick: () => load(), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(RefreshCw, { size: 16, className: loading ? "spinning" : "" }),
          " 分析"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("main", { children: !left || !right ? /* @__PURE__ */ jsxRuntimeExports.jsx(EmptyState, { loading: catalogLoading || loading, error }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        error ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "error-banner", children: error }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "run-headings", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceHeader, { label: "运行 A", summary: left }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(EvidenceHeader, { label: "运行 B", summary: right })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "metric-strip", "aria-label": "运行指标", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "总耗时", left: left.durationMS, right: right.durationMS, format: formatDuration, unit: " ms", lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "LLM 延迟", left: left.latencyMS, right: right.latencyMS, format: formatDuration, unit: " ms", lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "Token 数", left: left.totalTokens, right: right.totalTokens, lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "模型调用", left: left.modelCalls, right: right.modelCalls, lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "工具调用", left: left.toolCalls, right: right.toolCalls, lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "错误数", left: left.errors, right: right.errors, lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "关键路径", left: left.criticalPathMS, right: right.criticalPathMS, format: formatDuration, unit: " ms", lowerIsBetter: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "产物数", left: left.artifactCount, right: right.artifactCount })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(TextEvidence, { title: "提示词", left: left.prompt, right: right.prompt, empty: "未记录提示词。" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(TextEvidence, { title: "运行结果", left: left.result, right: right.result, empty: "未记录运行结果。" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(TextEvidence, { title: "摘要", left: left.summary, right: right.summary, empty: "未记录摘要。" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(TraceEvidence, { left, right }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Diagnostics, { left, right }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Rubric, { evaluation, setEvaluation, onSave: saveEvaluation$1, onAuto: autoEvaluate$1, onReset: resetEvaluation, onExport: exportEvaluation, saved: savedAt, saving, judging }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(EvaluationHistory, { evaluations })
      ] }) })
    ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(BatchWorkspace, { catalog, initialLeftID: leftID, initialRightID: rightID })
  ] });
}
