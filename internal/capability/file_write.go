...
func writeLocalFileAtomic(ctx context.Context, request WriteFileRequest) (FileResult, error) {
	...
	if strings.TrimSpace(request.Path) == "" {
		return FileResult{}, newFileReadError("invalid_write_path", "write_file path is required", nil)
	}
	...
	if mode != WriteModeCreate && mode != WriteModeOverwrite && mode != WriteModeCreateOrOverwrite {
		return FileResult{}, newFileReadError("invalid_write_mode", "mode must be create, overwrite, or create_or_overwrite", map[string]any{"mode": request.Mode})
	}
	if request.ExpectedAbsent && request.ExpectedRevision != "" {
		return FileResult{}, newFileReadError("invalid_write_precondition", "expected_absent and expected_revision are mutually exclusive", nil)
	}
	...
	if expected := strings.ToLower(strings.TrimSpace(request.ContentSHA256)); expected != "" && expected != checksum {
		return FileResult{}, newFileReadError("content_checksum_mismatch", "content_sha256 does not match content", map[string]any{"expected_sha256": expected, "actual_sha256": checksum})
	}
	...
	if mode == WriteModeCreate && exists || request.ExpectedAbsent && exists {
	...
	if mode == WriteModeOverwrite && !exists {
	...
	if request.ExpectedAbsent && currentExists || mode == WriteModeCreate && currentExists {
	...
	if mode == WriteModeOverwrite && !currentExists {
	...
	if mode == WriteModeCreate || request.ExpectedAbsent {
		...
		_ = os.Remove(temporaryPath)
	} else if err := os.Rename(temporaryPath, request.Path); err != nil {
