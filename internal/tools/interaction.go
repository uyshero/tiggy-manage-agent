...
func (InteractionRuntime) Manifest() Manifest {
	return Manifest{
		...
		SystemRole:     `Most tasks do not need a clarification. Use interaction_ask_user only when missing information materially changes the result, scope, cost, or an irreversible next action and the answer cannot be obtained from available context or read-only tools. Do not ask when a safe reversible default is available: state the assumption and continue. Do not use ask_user for progress updates, tool approval, plan approval, or file upload. Ask one focused question at a time and prefer concrete choices when the options are known. Use form mode when the user needs to provide several explicit fields such as environment, owner, deadline, rollout window, URLs, ticket IDs, or configuration values. Use interaction_request_upload only when the task genuinely requires binary files or large attachments that cannot be provided as text fields or existing session artifacts. Use interaction_request_plan_approval only when the user must review the current active task plan before execution. Plan approval approves direction only: it never approves a later tool call, command, or side effect.`,
