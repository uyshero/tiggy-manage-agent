...
func (DefaultRuntime) Manifest() Manifest {
		...
		API: []API{
			{
				...
				Parameters:     json.RawMessage(`{"type":"object","additionalProperties":false,"properties":{"command":{"type":"string","minLength":1},"args":{"type":"array","items":{"type":"string"}},"work_dir":{"type":"string","x-tma-file-ref":"directory"},"env":{"type":"object","additionalProperties":{"type":"string"}},"stdin":{"type":"string"},"timeout_ms":{"type":"integer","minimum":100,"maximum":600000,"default":120000,"description":"Total command deadline in milliseconds."},"max_output_bytes":{"type":"integer","minimum":1024,"maximum":1048576,"default":65536,"description":"Maximum captured bytes for each of stdout and stderr. Additional bytes are counted and discarded."},"output_paths":{"type":"array","items":{"type":"string","x-tma-file-ref":"output"},"description":"Optional final file paths to persist as session artifacts after the command succeeds. In cloud_sandbox, final deliverables must be under /workspace, for example /workspace/report.csv. Do not publish temporary files."}},"required":["command"]}`),
			...
			},
			{
				...
				Parameters:      json.RawMessage(`{"type":"object","additionalProperties":false,"properties":{"language":{"type":"string"},"code":{"type":"string"},"work_dir":{"type":"string","x-tma-file-ref":"directory"},"env":{"type":"object","additionalProperties":{"type":"string"}},"timeout_ms":{"type":"integer","minimum":100,"maximum":600000,"default":120000},"max_output_bytes":{"type":"integer","minimum":1024,"maximum":1048576,"default":65536},"output_paths":{"type":"array","items":{"type":"string","x-tma-file-ref":"output"},"description":"Optional final file paths to persist as session artifacts after the code finishes. In cloud_sandbox, final deliverables must be under /workspace, for example /workspace/report.csv. Do not publish temporary files."}},"required":["language","code"]}`),
				ApprovalPolicy:  ApprovalPolicyAlways,
				ApprovalReason:  InterventionReasonProcessExec,
			...
			},
			{
				...
				Parameters:     json.RawMessage(`{"type":"object","additionalProperties":false,"properties":{"root":{"type":"string","x-tma-file-ref":"directory","description":"Directory used as the path-pattern root; defaults to the workspace root."},"query":{"type":"string","minLength":1,"maxLength":1024,"description":"Single-line text or Go RE2 expression to search for."},"paths":{"type":"array","x-tma-file-ref":false,"items":{"type":"string"},"minItems":1,"maxItems":32,"description":"Relative path patterns, for example internal/**/*.go or README.md."},"exclude":{"type":"array","items":{"type":"string"},"maxItems":32},"mode":{"type":"string","enum":["literal","regex"],"default":"literal"},"case_sensitive":{"type":"boolean","default":true},"include_hidden":{"type":"boolean","default":false},"max_files":{"type":"integer","minimum":1,"maximum":5000,"default":1000},"max_results":{"type":"integer","minimum":1,"maximum":500,"default":100}},"required":["query","paths"]}`),
				Capabilities:   []string{CapabilityFilesystemRead},
				Risk:           ToolRiskRead,
				Runtime:        &RuntimePolicy{Allowed: []string{ToolRuntimeAuto, ToolRuntimeCloudSandbox, ToolRuntimeLocalSystem}, Preferred: ToolRuntimeAuto},
				Implementation: ToolImplementationWorkerCapability,
			},
			{
				...
				Parameters:      json.RawMessage(`{"type":"object","additionalProperties":false,"properties":{"path":{"type":"string","x-tma-file-ref":"read","description":"Text file path or FileRef URI to search. Cloud sandbox path rules are identical to read_file."},"query":{"type":"string","minLength":1,"maxLength":1024,"description":"Single-line literal UTF-8 string, not a regular expression."},"max_results":{"type":"integer","minimum":1,"maximum":100,"description":"Maximum matching lines to return; defaults to 50."},"file_revision":{"type":"string","description":"Optional revision from read_file. A mismatch returns stale_file_revision."}},"required":["path","query"]}`),
				Capabilities:    []string{CapabilityFilesystemRead},
				Risk:            ToolRiskRead,
				Runtime:         &RuntimePolicy{Allowed: []string{ToolRuntimeAuto, ToolRuntimeCloudSandbox, ToolRuntimeLocalSystem}, Preferred: ToolRuntimeAuto},
				Implementation:  ToolImplementationWorkerCapability,
	...
	manifest.SystemRole = strings.ReplaceAll(
		manifest.SystemRole,
		"Use /mnt/data only for caches, temporary files, and intermediate generation results. If a final result was built under /mnt/data, copy or move it into /workspace and publish only the /workspace path before completion.",
		"Use /mnt/data or the session-shared /tmp only for caches, temporary files, and intermediate generation results. TMPDIR, TMP, and TEMP already point to /tmp. If a final result was built under /mnt/data or /tmp, copy or move it into /workspace and publish only the /workspace path before completion.",
	)
	manifest.SystemRole = strings.ReplaceAll(
		manifest.SystemRole,
		"Absolute file paths must stay under /workspace or /mnt/data; do not use /root, /tmp, or other absolute roots.",
		"File arguments accept legacy paths and fileref://workspace, fileref://data, fileref://tmp, or fileref://artifact URIs. Absolute file paths must stay under /workspace, /mnt/data, or /tmp; do not use /root or other absolute roots.",
	)
	return manifest
}
	...
	case "write_file":
		...
		return ExecutionResult{
			...
			ExportedFiles: []ArtifactExport{{
				...
				Description:  "Generated by default_write_file",
				ArtifactType: "file",
			}},
		}, nil
	case "edit_file":
...
func editFileResult(call Call, result capability.EditFileResult) (ExecutionResult, error) {
	...
	if !result.Success {
		...
		executionResult.ExportedFiles = []ArtifactExport{{
			Path:         result.Path,
			Name:         filepath.Base(result.Path),
			Description:  "Generated by default_edit_file",
			ArtifactType: "file",
		}}
	}
