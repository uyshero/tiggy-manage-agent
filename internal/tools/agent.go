...
func (AgentRuntime) Manifest() Manifest {
	return Manifest{
		...
		SystemRole:     "Use agent_* tools only when there are two or more independent subtasks that can run concurrently, specialized isolation is useful, or intermediate research would consume substantial parent context. Do not delegate single-step work, tightly coupled sequential work, or tasks that can be completed in one or two tool calls. Prefer agent_spawn for one independent unit and agent_run_group for parallel fan-out. Always wait for and collect delegated results before final completion.",
