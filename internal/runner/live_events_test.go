...
func TestLiveEventBrokerPublishesTextlessReset(t *testing.T) {
	broker := NewLiveEventBroker(1)
	events, cancel, err := broker.SubscribeLiveEvents("session")
	if err != nil {
		t.Fatal(err)
	...
	broker.Publish(LiveEvent{SessionID: "session", TurnID: "turn", Type: LiveEventLLMText, Operation: "reset"})
	select {
	case event := <-events:
		if event.Operation != "reset" || event.Text != "" || event.StreamSeq == 0 {
