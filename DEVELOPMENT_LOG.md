...
- CLI `event stream` 对 `runtime.tool_intervention_required/approved/rejected` 和 `runtime.tool_result` 的可读展示
...
sql/migrations/000014_session_intervention_expires_at.sql
```
...
### Tool intervention 已有 pending 队列和非交互式决策入口
...
- pending intervention 写入 `expires_at`
...
因此当前能力已经能“拦住、挂起 turn、持久化、让人用命令做决策、批准后执行工具、把结果送回 LLM 并完成本轮”。剩余主要是多轮 continuation、usage 记录和更好的交互入口。

长期无人审批时，第一版不会自动 approve / reject / expire。pending 记录会保留在 `session_interventions` 中，可继续通过 list 查询并由用户后续决策。后续如果需要超时，可在记录上增加 `expires_at`，再按 Session/Agent 策略做 `auto_reject` 或继续保持 pending。
