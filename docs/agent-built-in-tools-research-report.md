...
4. **通用企业 Agent 建议常驻 8～10 个工具 schema，其中只有 3～5 个最高频工具保持“热加载”。** 浏览器全集、Office/PDF、MCP、记忆、定时任务和子 Agent 按场景或检索结果加载。
...
12. **TMA 当前不是“从零开始”。** 它已经具备 schema 双重校验、分页 revision、结果上下文预算、审批续跑、MCP SSRF 防护、Skill 安装扫描和子 Agent 配额。近期最值得补的是渐进式工具暴露、更丰富风险元数据、结构化输出 schema、同步命令约束，以及文件写入的并发前置条件。
...
通用企业智能体建议默认注册以下 10 个工具 schema。这里的“默认”指常见会话可见，不代表无审批执行。
...
这 10 个工具只有在 schema 总量可控时才应全部直载。建议同时设置两道门槛：
...
| 执行 | `command_failed`、`timeout`、`output_limit_exceeded` | 取决于原因 |
...
系统不会把 `command` 和 `args` 重新拼成 shell 字符串。需要管道、重定向或变量展开时，调用方必须显式选择 `sh` 和 `-c`，并接受相应的命令执行风险。
...
当前 `run_command` 只接受有明确起止的同步任务。达到 deadline 后必须失败并清理整个进程组，不能把子进程留在后台。

TMA 当前已实现：默认 120 秒、范围 100 毫秒至 10 分钟的 `timeout_ms`；stdout/stderr 每路默认 64 KiB、硬上限 1 MiB；总字节数、捕获字节数、截断标记、duration 和 timeout 状态进入结构化结果；Unix 本机执行在独立进程组中运行并在超时后整组终止。Session 云沙箱会在命令 timeout/cancel 后强制删除容器，清理失败则失败关闭；它同时已有 CPU、内存和 PID 容器限制。本机 Worker 不具备同等级资源隔离，只应在受信任环境显式启用。

### 7.2 暂不提供 `terminal` 和 `process`

持久终端意味着 TTY、连续 stdin、终端尺寸、Shell 状态和更高权限边界；后台进程意味着 process handle、日志 cursor、wait/kill、TTL、重启恢复和孤儿进程清理。当前基础工具阶段没有这些明确需求，因此不应为可能的未来场景增加两个默认 Schema。

只有业务出现“命令返回后仍需运行，并要跨轮次读日志、写 stdin 或终止”的稳定需求时，才增加独立 `process`；交互式 TTY 需求出现前不增加 `terminal`。

### 7.3 `execute_code` 只保留显式兼容

如果 `execute_code` 与 `run_command` 使用相同容器、文件权限、网络、环境变量和审批策略，它只是 `python -c` 的包装，没有形成新的安全边界，却增加了工具选择歧义和 Schema 成本。因此 TMA 当前保留 executor 和显式 Agent config 兼容，但不放入默认模型工具集。

只有建成独立受限代码沙箱后才重新默认开放：

- 固定解释器和预装依赖，不提供 Shell 或包管理器；
- 默认无网络、无业务凭据、只读输入和独立临时目录；
- 只允许写入声明的输出目录，扫描后再发布 Artifact；
- 使用独立 CPU、内存、PID、wall-clock 和输出配额；
- 以非 root、drop capabilities、no-new-privileges 和受限系统调用运行。

工具名称本身不提供隔离。Claude Code 的当前实践说明，命令审批和 OS 级沙箱应叠加；Hermes 的多后端设计则说明执行环境是 provider 能力，不应写死在工具名称里。[2][10]
...
| P1 | 多个输入 schema 未关闭未知字段 | `run_command` 与兼容 `execute_code` 已关闭未知字段；`write_file`、`edit_file`、`web.*` 尚未统一 | 统一 schema linter，在 CI 拒绝未显式声明的对象策略 |
...
- argv 与 shell 注入边界。
- timeout 后子进程组清理。
- stdout/stderr 洪泛、二进制输出和截断元数据。
- CPU/内存/PID/磁盘限制。
- 网络禁用、域 allowlist、凭据环境变量不可见。
- timeout、取消或服务退出后的子进程残留检查。
...
### Phase 2：收紧同步命令与结果生命周期

交付：

- 补齐 Windows Job Object、磁盘配额和流式日志。
- `execute_code` 保留显式配置兼容，但不进入默认模型工具集。
...
2. 收紧文件写、命令、网络和凭据边界。
3. 补同步命令的取消、超时、进程组清理和 Artifact 生命周期。
