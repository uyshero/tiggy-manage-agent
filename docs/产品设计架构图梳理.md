...
TMA 当前把 `cloud_sandbox` runtime 落到 `OnlyboxesProvider`。当前实现通过本地 Docker 隔离执行工具命令：每次工具调用按需 `docker run --rm` 启动一次性容器，挂载 workspace 到 `/workspace`，命令结束后容器删除。TMA 不自动启动 Docker daemon 或 Onlyboxes Console；Onlyboxes Console 可作为后续 Remote Provider 的执行后端。
