...
| `browser.*` | 浏览器导航、读取、交互、截图与人工接管 | Browser Worker Process Plugin；`local_system` |
...
| `run_command` | `exec` | `exec` | `cloud_sandbox` | 同步；默认 120s；stdout/stderr 各默认 64 KiB；local_system 需显式允许 |
| `execute_code` | `code.execute` | `exec` | `cloud_sandbox` | 兼容 API，默认对模型隐藏；显式配置后可用 |
...
Browser 能力域由外置 Browser Gateway 与 Worker Process Plugin 提供，不属于 `tma-server` 默认 Registry。Agent 只有在同 Workspace 的 Browser Worker 在线且配置启用 `browser` 时才能看到这些工具。

| API | capabilities | risk | runtime 默认 | 第一版实现 |
|---|---|---|---|---|
| `open` | `browser.open`, `browser.read` | `read` | `local_system` | Browser Tool Plugin 调用外置 Gateway |
| `navigate` | `browser.navigate` | `read` | `local_system` | 暂并入 `open` 的 `url` 参数 |
| `read` | `browser.read` | `read` | `local_system` | Gateway 返回正文和可交互元素 |
| `click` | `browser.read`, `browser.interact` | `write` | `local_system` | Gateway Session 内串行执行 |
| `type` | `browser.read`, `browser.interact` | `write` | `local_system` | Gateway Session 内串行执行 |
| `screenshot` | `browser.read`, `browser.capture` | `read` | `local_system` | Process Plugin 输出 JPEG artifact |
| `takeover` | `browser.read`, `browser.takeover` | `write` | `local_system` | 返回 Workbench Browser Extension 路由 |
| `close` | `browser.close` | `write` | `local_system` | 关闭 Gateway Chromium Session |
| `download` | `browser.download`, `artifact.write` | `write` | `local_system` | Gateway 扩展待实现 |
| `upload_file` | `browser.upload`, `filesystem.read` | `write` | `local_system` | Gateway 扩展待实现 |
| `network_log` | `browser.network` | `read` | `local_system` | Gateway 扩展待实现 |
