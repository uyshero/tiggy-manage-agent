...
`openai-compatible` 当前支持 Chat Completions 文本响应、六类 SSE chunk、原生 tool calling 适配和 usage 归集。普通文本流式输出会写入 `runtime.llm_chunk(type=text)` 和兼容用 `runtime.llm_delta`；其他类型只写统一 chunk。tool-call fragment 仅用于观测，Runtime 等 Provider 组装完最终 `Response.Message.ToolCalls` 后才执行工具；流内 error 会先写 error chunk，再让 turn 进入统一失败路径。最终文本仍合并成一个完整 `agent.message`。Key Vault 和完整 model-bank 仍待做。此处先固定边界：Provider 负责创建模型 client，Manager 负责根据请求选择 Provider / Model，Runtime 只依赖 `llm.Client`。
