...
当前 `default.read_file` 默认直通；`run_command`、`execute_code`、`write_file`、`edit_file` 会进入这套 policy。
...
- AgentConfigVersion 的 `skills` 非空时作为 system 级上下文注入，不参与 provider 选择。
...
- system、pinned context、summary 和当前 user message 是保底消息，不会被删除。
...
- token 数当前是近似估算：每条 message 有固定开销，文本按约 4 个字符 1 token 估算，tool schema 按 JSON 序列化后的文本估算。
- `runtime.llm_request` step 会记录 `history_count`、`omitted_history_count`、`estimated_token_count`、`estimated_message_tokens`、`estimated_tool_schema_tokens`、`tool_schema_count`、`context_truncated`、`pinned_context_included` 和 `summary_included`，并在 `context_budget` 中给出 `context_window_tokens`、`input_budget_ratio_percent`、`max_input_tokens`、`reserved_output_tokens`、`message_tokens`、`pinned_context_tokens` 和 `tool_schema_tokens` 等分账。其中 `estimated_token_count` 包含 messages 与原生 function calling tool schema 的估算成本。
