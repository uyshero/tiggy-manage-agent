...
| --- | --- | --- | --- |
| LobeHub | [`packages/local-file-shell/src/file/read.ts`](https://github.com/lobehub/lobehub/blob/929f2519c3a29fc316792af59a4e9382d6258583/packages/local-file-shell/src/file/read.ts), [`systemRole.ts`](https://github.com/lobehub/lobehub/blob/929f2519c3a29fc316792af59a4e9382d6258583/packages/builtin-tool-local-system/src/systemRole.ts) | 对模型提供 `loc: [start,end]`，默认 200 行，限制超长单行和总输出，并指导先 grep；但 `loadFile()` 后再 `split('\n')`，10 MB 内仍是整文件加载。 | 采用行范围、超长单行提示和搜索优先工作流；拒绝整文件加载后切片，也不提供可绕过预算的 `fullContent`。 |
