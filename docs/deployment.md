...
新空库使用当前基线 `sql/baselines/000088_baseline.sql`；已有库只应用更高编号的
...
psql "$TMA_MIGRATION_DATABASE_URL" \
  -v ON_ERROR_STOP=1 \
  --single-transaction \
  -f sql/baselines/000088_baseline.sql
```
...
git diff --exit-code -- sql/baselines/000088_baseline.sql
...
kubectl -n tma wait --for=condition=complete job/tma-database-baseline-000088 --timeout=10m
kubectl -n tma logs job/tma-database-baseline-000088
```
...
- [ ] `000088` 基线或增量 migration 成功，应用不持有 owner 凭据。
