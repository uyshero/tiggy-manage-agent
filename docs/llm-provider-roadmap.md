...
它只表达文本增量。统一 chunk 已完成六类分型：

```text
runtime.llm_chunk
  type: text | reasoning | tool_call | usage | stop | error
  data: typed payload
```

迁移策略：

1. 已保留 `runtime.llm_delta`，兼容现有验收脚本。
2. 已新增 `runtime.llm_chunk`，接入 OpenAI-compatible `reasoning_content` / `reasoning_details`、tool-call fragments、usage、finish reason / `[DONE]` 和流内 error。
3. `runtime.llm_delta` 只为 text 保留兼容双写；tool-call fragment 不触发执行，成功响应流开始后的解码错误不重试。
...
5. 统一 `runtime.llm_chunk`：`text | reasoning | tool_call | usage | stop | error` 六类分型已完成。
...
7. Provider 错误归一化与有界重试：`ProviderError` 已提供 `auth`、`rate_limit`、`context_length`、`timeout`、`server`、`invalid_request`、`unknown` 类别、`Retryable` / `RetryAfter` / `Attempts` 语义；OpenAI-compatible 调用已支持 `Retry-After` 和可配置指数退避，成功响应流开始后不重放。Router / failover 仍待实现。

当前进展：Context Builder 已从 `DemoRuntime` 中抽离，负责把 summary、`tools.Registry.ModelContext()` 生成的 manifest、skills、当前 `user.message` 之前的 `user.message` / `agent.message` 注入 LLM messages；模型总窗口由 `llm_models.context_window_tokens` 提供，Context Builder 固定使用 60% 作为输入预算；`session_summaries` 已提供手动 summary 和 just-in-time 自动 summary 落库；`internal/tools` 已提供内置工具 manifest / registry / executor 的最小闭环，并内置 `default`；Runtime 已支持 `llm.Request.Tools` / `llm.Message.ToolCalls` 原生工具调用路径，`openai-compatible` 已把它适配到 Chat Completions `tools` / `tool_calls`，同时保留 `tma.tool_call.v1` 文本工具调用 envelope 作为 fallback；统一流事件已覆盖六类 typed chunk；LLM usage 会从 `llm.Client` 返回，经 `AgentRuntimeTurnExecutor` 补齐 workspace / agent / session / turn / provider / model 维度后，由 `WorkerRunner` 写入 `llm_usage_records`。后续真实 tokenizer、多模态上下文和更多 Provider 的原生 tool calling 适配应继续在 Runtime 边界内演进。
