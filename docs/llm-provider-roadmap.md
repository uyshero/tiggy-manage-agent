...
Runtime 已接入 OpenAI-compatible `reasoning_content` / `reasoning_details`、tool-call fragments、usage、finish reason / `[DONE]` 和流内 error。原始 delta 不持久化；text 进入临时 Live SSE，其余内容只参与运行期组装和聚合。tool-call fragment 不触发执行，成功响应流开始后的解码错误不重试。
...
5. 统一 Provider delta：`text | reasoning | tool_call | usage | stop | error` 六类分型已完成，持久层只保存 `runtime.llm_response` 聚合。
