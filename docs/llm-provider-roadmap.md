...
1. Context Builder：把 Session 历史消息组装进 LLM request。
2. Usage 基础结构：`llm.Usage`、`llm_usage_records`、每 turn 落库。
3. OpenAI-compatible usage 解析：非流式和流式最终 usage。
4. Model abilities：`llm_models` 和能力 JSON。
5. 统一 `runtime.llm_chunk`。
6. Tool Calling。
7. Provider Router / failover。

当前进展：Runtime 已经能把当前 `user.message` 之前的 `user.message` / `agent.message` 注入 LLM messages。后续仍建议把消息组装逻辑从 `DemoRuntime` 中抽成独立 Context Builder，方便处理 token budget、截断、summary 和多模态。
