...
func TestCoreOperationsUseExplicitSchemas(t *testing.T) {
	...
	tests := []struct {
		...
		{"get", "/v2/object-cleanup/jobs", "", "200", "#/components/schemas/ObjectCleanupJobList", "application/json"},
		{"get", "/v2/object-cleanup/stats", "", "200", "#/components/schemas/ObjectCleanupStats", "application/json"},
		{"post", "/v2/object-cleanup/jobs/{job_id}/retry", "", "200", "#/components/schemas/ObjectCleanupJob", "application/json"},
		{"post", "/v2/object-cleanup/jobs/{job_id}/approve", "#/components/schemas/ApproveObjectCleanupRequest", "200", "#/components/schemas/ObjectCleanupJob", "application/json"},
		{"post", "/v2/object-cleanup/reconciliation/preview", "#/components/schemas/ObjectReconciliationPreviewRequest", "200", "#/components/schemas/ObjectReconciliationReport", "application/json"},
...
func TestApplicationCompatibilityRoutesStayOutOfCoreContract(t *testing.T) {
	contract, err := os.ReadFile("openapi.yaml")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, prefix := range []string{
		...
		for path := range paths {
			if strings.HasPrefix(path, prefix) {
				t.Errorf("application path %s must not be published by Core OpenAPI", path)
			}
		}
	}
	components := specification["components"].(map[string]any)
	...
	for name := range schemas {
		if strings.HasPrefix(name, "WorkbenchProject") || strings.HasPrefix(name, "KnowledgeService") ||
			strings.HasPrefix(name, "KnowledgeShare") {
			t.Errorf("application schema %s must not be published by Core OpenAPI", name)
		}
	}
}
...
func excludedV2Route(method string, path string) bool {
	if strings.HasPrefix(path, "/v2/workbench-projects") {
		return true
	}
