...
Options:
  ...
  --init-db                         Apply the 000087 baseline Job once.
...
if [[ "$INIT_DB" -eq 1 ]]; then
  ...
  if kubectl -n "$NAMESPACE" get job tma-database-baseline-000087 >/dev/null 2>&1; then
    complete="$(kubectl -n "$NAMESPACE" get job tma-database-baseline-000087 -o jsonpath='{.status.conditions[?(@.type=="Complete")].status}')"
    ...
    if ! kubectl -n "$NAMESPACE" wait --for=condition=complete job/tma-database-baseline-000087 --timeout="$TIMEOUT"; then
      kubectl -n "$NAMESPACE" logs job/tma-database-baseline-000087 --all-containers=true >&2 || true
      exit 1
    fi
    kubectl -n "$NAMESPACE" logs job/tma-database-baseline-000087 --all-containers=true
  fi
